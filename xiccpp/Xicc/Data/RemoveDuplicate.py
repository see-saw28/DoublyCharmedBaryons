...
def generateSavedList(pids, keys, vectors):
    ...
    if(len(keys)!=len(vectors)):
        raise Exception("the keys and vetors don't have the same length")
    totCan = len(keys)
...
mychain = TChain("tuple_sel_rec/DecayTree")


mychain.Add(f"/eos/lhcb/user/p/pgaigne/STEP3/{SAMPLE}/RS/Xiccpst-RS-{SAMPLE}.root")
tree_temp = mychain

tree_temp.SetName("temp")

fn=TFile(f"/eos/lhcb/user/p/pgaigne/STEP2/{SAMPLE}/Xiccpp-RS-{SAMPLE}-MVA-duplicate.root","recreate")
