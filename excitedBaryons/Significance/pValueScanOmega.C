...
//   list->Add(newtree2018);
  ...
  Int_t bkg_expected = newtree->GetEntries("C_KaonDTF_C_M > 4115. & C_KaonDTF_C_M < 4615.");
 ...
 RooGenericPdf bkg("bkg","BKG", "TMath::Power(x,power)*exp(-x*slope)",RooArgSet(x, slope, power)) ;
  ...
  graphPvalues->GetHistogram()->GetXaxis()->SetTitle("#it{m} (#it{#Xi_{cc}^{++}K^{-}}) (MeV/#it{c}^{2})");
  graphPvalues->GetHistogram()->GetYaxis()->SetTitle("local #it{p}-value");
  graphPvalues->GetHistogram()->GetXaxis()->SetRangeUser(mMin,mMax-1.);
  graphPvalues->GetHistogram()->GetYaxis()->SetRangeUser(1e-6,1.2);
  ...
  c1->SaveAs("pValuePlotRS_Omega.pdf");
  ofstream textfilePvalues;
  textfilePvalues.open("PvaluesOmega.txt"); 
  ...
  textfileMass.open("massOmega.txt");
...
// plot minimal value fit
  ...
  c1_Data->SaveAs("pScan_XiccK_fit_RS.pdf");
