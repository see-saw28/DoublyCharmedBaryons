...
void FitGaussDSCBcontrolNewInclusivePaper(){
  ...
  model.plotOn(xframe,Name("bkgmodel"),Components("bkg"),LineStyle(kDotted),LineColor(kGreen+2));
  ...
  TLegend *leg = new TLegend(0.65,0.91,0.92,0.64);
  leg->SetBorderSize(0);
  leg->AddEntry(xframe->findObject("dataset"),"Data","lep");
  ...
  xframe->GetXaxis()->SetTitle("#it{m} (#it{#Xi_{c}^{+}#pi^{+}}) [MeV/#it{c}^{2}]");
  xframe->GetYaxis()->SetRangeUser(0,300);
...
*/
  ...
  lat.DrawLatex(0.19,0.86,"LHCb");
  lat.DrawLatex(0.19,0.78,"5.4 fb^{#font[122]{-}1}");
  xframe->SetTitle("");

  c->cd();          // Go back to the main canvas before defining pad2

  c->SaveAs("Xiccpp_GaussDSCB_RSdata_combined_InclusiveL0TIS_Paper.pdf");
  c->SaveAs("Fig5.pdf");
  c->SaveAs("Fig5.eps");
  c->SaveAs("Fig5.C");
  c->SaveAs("Fig5.png");
