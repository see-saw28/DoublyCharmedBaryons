...
paths = ["/eos/lhcb/user/p/pgaigne/job138-Xiccpst-2016-MD-0.root",
...
paths = ["/eos/lhcb/user/p/pgaigne/STEP2/2016/Xiccpp-RS-2016-MVA-duplicate.root",
         "/eos/lhcb/user/p/pgaigne/STEP2/2017/Xiccpp-RS-2017-MVA-duplicate.root",
         "/eos/lhcb/user/p/pgaigne/STEP2/2018/Xiccpp-RS-2018-MVA-duplicate.root"]

paths = ["/eos/lhcb/user/p/pgaigne/STEP2/RetrainedNtuples/Xiccpp-Data-Run2-Lc-clone-duplicate.root"]
...
for path in paths :
    ...
    branches_we_want = ["Xicc_M_DTF_Lc_PV","Lc_M","Xicc_TMVA_BDTXicc"] 
...
mva_methods={
             'Xicc_TMVA_BDTXicc' :{'min_cut':0.0,'max_cut':0.25,'nb_step':26}
             }
...
for k,mva_method in enumerate(mva_methods) :
    ...
    for i,mva_cut in enumerate(mva_cuts):
        ...
        plt.savefig(f"mva_scan/after-removal/fit-{mva_method}-{str(mva_cut).replace('.','-')}-{year}.png",bbox_inches="tight")
        plt.close()
    ...
    plt.axvline(x=opt_cut[k],color='red')
    axes = plt.gca()
    axes.set_xlim([min_cut,max_cut])
    plt.savefig(f"mva_scan/after-removal/mva_scan-{mva_method}-{year}.png",bbox_inches="tight")   # the model as the sum of the individual pdfs
...
axes = plt.gca()
axes.set_xlim([0,1])
plt.savefig(f"mva_scan/after-removal/mva_scan-all-methods-{year}.png",bbox_inches="tight")   # the model as the sum of the individual pdfs
