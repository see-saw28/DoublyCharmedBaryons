...
def Setup_options( the_year , polarity, evt_id, ganga=True ):
    evt_id = int(evt_id)
    ...
    from Configurables import GaudiSequencer, CombineParticles, NoPIDsParticleMaker
    from PhysSelPython.Wrappers import Selection, SelectionSequence
    from PhysSelPython.Wrappers import DataOnDemand, AutomaticData
    # from GaudiKernel.SystemOfUnits import *
    from PhysConf.Selections import CheckPVSelection, ValidBPVSelection, FilterSelection
    ...
    if the_year == '2016':
        ...
        pi_proto = NoPIDsParticleMaker("TurboAllNoPIDsPions", Particle="pion", Input="Long/Protos")
        Pions = FilterSelection("pi", [AutomaticData("Phys/TurboAllNoPIDsPions/Particles")], Code="(P>1*GeV)")

        k_proto = NoPIDsParticleMaker("TurboAllNoPIDsKaons", Particle="kaon", Input="Long/Protos")
        Kaons = FilterSelection("kaon", [AutomaticData("Phys/TurboAllNoPIDsKaons/Particles")], Code="(P>1*GeV)")
    ...
    TeslaTruthUtils.makeTruth(Xiccp_REC, relations, mc_tools)

    XiccXmlDiff = "TMVAClassification_BDT.weights.xml"
    ...
    TeslaTruthUtils.makeTruth(Xiccp_REC_WS, relations, mc_tools)
    ...
    if the_year == '2016':
        DaVinci().UserAlgorithms = [seq3.sequence(),seq5.sequence()]#, seq4.sequence()]#,seq5.sequence()]
    else :

        DaVinci().UserAlgorithms = [pi_proto, k_proto, seq3.sequence()]#, seq4.sequence()]#,seq5.sequence()]

     
    
    DaVinci().InputType = 'MDST' 
    DaVinci().TupleFile = f'DV_Xiccp_MC_{evt_id}_{the_year}_{polarity}.root' 
...
if test:            
    ...
    Setup_options( the_year , polarity, evt_id, ganga=False )
