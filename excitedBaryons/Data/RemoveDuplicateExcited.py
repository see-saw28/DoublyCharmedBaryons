...
def get_pids_keys_trackvs(tree):
    ...
    trackv_Pi = TVector3( tree.Pi_PX, tree.Pi_PY, tree.Pi_PZ)
    ...
    for ii in range(totCan-1):
        ...
        for jj in range(ii+1,totCan):
            ...
            if (SAMPLE==2016):        
                ...
                if len(set(key1)) !=7 or len(set(key1)) !=7: 
...
for baryon in baryons :
    for SAMPLE in years :
        ...
        path = f"/eos/lhcb/user/p/pgaigne/STEP3/{SAMPLE}/RS/{baryon}-RS-{SAMPLE}-Xicc-Lc-Loose-clone.root"
        # path = f"/eos/lhcb/user/p/pgaigne/STEP2/RetrainedNtuples/Xiccpp-Data-{SAMPLE}-Lc-clone.root "

        # path = f"/eos/lhcb/user/p/pgaigne/STEP3/{SAMPLE}/RS/{baryon}-RS-{SAMPLE}-Lc-Loose-clone.root"
